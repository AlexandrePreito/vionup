  ...
  try {
    ...
    try {
      ...
      if (force_full_sync) {
        ...
        // Ignora initial_date, usa período completo a partir da data inicial configurada
        syncType = 'full';
        ...
        if (isFirstSync && config.initial_date) {
          ...
          // Sincronização incremental (botão verde): últimos N dias
...
async function processInDateRangeBatches(
  ...
  if (syncType === 'incremental') {
    // Incremental: apenas últimos N dias (não usa filterDate, calcula diretamente)
    startDate = new Date();
    startDate.setDate(startDate.getDate() - (config.incremental_days || 7));
    console.log(`🟢 Sincronização INCREMENTAL: últimos ${config.incremental_days || 7} dias`);
  } else if (syncType === 'full' && filterDate) {
    // Completa forçada: do initial_date até hoje
...
async function saveValidData(
  ...
  console.log(`  📦 Processando ${totalBatches} lote(s) de salvamento (${validData.length} registros válidos)...`);
  console.log(`  ℹ️  Usando UPSERT: registros existentes serão atualizados, novos serão inseridos. Nenhum registro será deletado.`);
  ...
  for (let i = 0; i < validData.length; i += upsertBatchSize) {
    ...
    try {
      // UPSERT: atualiza registros existentes ou insere novos, sem deletar nada
      const { error: upsertError } = await supabaseAdmin
        .from(destTable)
        .upsert(batch, { onConflict: 'company_group_id,external_id', ignoreDuplicates: false });

      if (upsertError) {
        console.error(`  ❌ Erro ao salvar lote ${batchNumber}/${totalBatches}:`, upsertError.message);
        // Continuar com próximo lote mesmo se houver erro
      } else {
        savedCount += batch.length;
        if (batchNumber % 10 === 0 || batchNumber === totalBatches) {
          console.log(`  ✓ Lote ${batchNumber}/${totalBatches} salvo (${savedCount.toLocaleString()}/${validData.length.toLocaleString()})`);
        }
      }
    } catch (batchError: any) {
      console.error(`  ❌ Erro ao salvar lote ${batchNumber}/${totalBatches}:`, batchError.message || batchError);
      // Continuar com próximo lote mesmo se houver erro
    }
  }

  console.log(`  ✅ Salvamento concluído: ${savedCount.toLocaleString()} de ${validData.length.toLocaleString()} registros salvos`);
